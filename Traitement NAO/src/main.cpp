...
{
	string filename = "../../images/le-robot-nao.jpg";
	// Lire et afficher l'image
	Mat src = imread(filename, 0);
	if(src.empty())
	{
	...
	Mat traitement;
	double t = otsu("../../images/voilier_oies_blanches.jpg");
	threshold(src, traitement, t, 255, THRESH_BINARY);
	namedWindow("traitement", WINDOW_AUTOSIZE);
	moveWindow("traitement", 2*src.cols, 0);
	imshow("traitement", traitement);

	waitKey(0);

	//Ouverture fermeture de l'image binarisée
	Mat opening_structuringElement = getStructuringElement(MORPH_RECT,  Size(8, 8));
	Mat closing_structuringElement = getStructuringElement(MORPH_RECT,  Size(11, 11));
	morphologyEx(traitement, traitement, MORPH_OPEN, opening_structuringElement);
	morphologyEx(traitement, traitement, MORPH_CLOSE, closing_structuringElement);
	imshow("traitement", traitement);
	...
	vector<vector<Point>> contours;
	vector<Vec4i> hierarchy;
	findContours(traitement, contours, hierarchy, cv::RETR_TREE, cv::CHAIN_APPROX_NONE);
